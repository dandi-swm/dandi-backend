...
enum class AuthErrorCode(override val status: HttpStatus, override val code: String, override val message: String) :
    ...
    MAIL_RESEND_TOO_EARLY(
        ...
        "api.auth.mail-resend-too-early",
        "인증 코드를 발송한 지 5분이 지나지 않았습니다.",
    ),
    MAIL_NOT_FOUND(HttpStatus.NOT_FOUND, "api.auth.mail-not-found", "해당 이메일로 발송된 인증 코드가 없습니다."),
    MAIL_CODE_EXPIRED(HttpStatus.BAD_REQUEST, "api.auth.mail-code-expired", "인증 코드 유효 시간이 지났습니다. 코드를 재발송 받으세요."),
    MAIL_CODE_MISMATCH(HttpStatus.BAD_REQUEST, "api.auth.mail-code-mismatch", "인증 코드가 일치하지 않습니다."),
